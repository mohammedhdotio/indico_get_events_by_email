...
from indico_get_events_by_email.utils import build_event_dict
...
class RHGetEventsByEmailV2(RH):
    ...
    def _process(self):
        ...
        managed_events = build_event_dict(get_events_managed_by(user))

        linked_events = build_event_dict(get_events_with_linked_event_persons(user))

        return jsonify(user_id=user.id, managed_events=managed_events, linked_events=linked_events)
